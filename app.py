...
def confirm_clear_knowledge_base():
    st.write("This permanently deletes **all** indexed documents and vectors. This can't be undone.")
...
with st.sidebar:
    ...
    st.caption("Upload failed? Reload the page and try again — this can happen right after the app wakes up.")
        ...
        with progress_container:
            ...
            try:
                ...
                if all_chunks:
                    ...
                    get_vector_store().create_index(all_chunks)
        ...
        if "sources" in message and message["sources"]:
            with st.expander(f"📚 {len(message['sources'])} sources", expanded=False):
                for i, source in enumerate(message["sources"], 1):
                    relevance = f" • {source['score']:.0%} relevance" if source.get('score') is not None else ""
                    st.markdown(f"**{i}. {source['source']}** • Chunk {source['chunk_id']}{relevance}")
                    with st.container():
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            try:
                ...
                results = get_vector_store().search(prompt, k=3)
                ...
                if not results:
                    ...
                    response = get_rag_chain().query(prompt, results)
